...
N_Prediction_Horizon = 5;  % Number of step of prediction horizon
...
%filenameVal = 'Vinf8dot5_sowfa_2turb_alm_turbl'; %'Vinf8dot5_sowfa_2turb_alm_turbl_AllComb.mat';
%filenameId = 'Vinf8dot0_sowfa_2turb_alm_turbl_AllComb.mat'; NTMstr = '';
%filenameId = 'Vinf8dot0To8dot5_sowfa_2turb_alm_turbl_AllComb.mat';
% filenameId = 'Vinf7dot0_sowfa_2turb_alm_turbl_AllComb.mat'; NTMstr = '';
% filenameVal = 'Vinf7dot0To9dot0_sowfa_2turb_alm_turbl_AllComb.mat'; NTMstr = '';
NTMstr = '';
%filenameVal =  'Vinf8dot0_sowfa_2turb_yaw_alm_turbl_AllComb1.mat'; %'Vinf7dot9_Stairs1sowfa_2turb_yaw_alm_combined2.mat';
%filenameVal = 'Vinf11dot0_sowfa_2turb_yaw_alm_combined2';
...
%filenameVal = 'Vinf8dot0_sowfa_2turb_yaw_steps'; %'Vinf8dot0_sowfa_2turb_yaw_alm_turblpos';%'Vinf8dot0_Stairs1sowfa_2turb_yaw_alm_turbl';%'Vinf8dot5_sowfa_2turb_alm_turbl.mat';

%filenameVal = 'Vinf8dot0_sowfa_2turb_yaw_noise_step';
filenameVal =  'Vinf8dot0_sowfa_2turb_alm_turbl.mat'; %
...
for idx = 1:length(fieldCell)
    if useVal
        idxPhiVal = tmpVal.phi1 <= 30;
        if length(tmpVal.(fieldCell{idx})) == length(idxPhiVal)
            tmp = [tmpId.(fieldCell{idx}), tmpVal.(fieldCell{idx})(idxPhiVal)]; %#ok<NASGU>
        else
            tmp = [tmpId.(fieldCell{idx}), tmpVal.(fieldCell{idx})];
        end
    else
...
% phiTemp = cos(phi1/180 *pi); %phi1/180 *pi; %
% phi1 = (phiTemp - mean(phiTemp))/std(phiTemp);
...
ptemp = reshape(p,Wp.mesh.Nx,Wp.mesh.Ny,kk);
ptempT = squeeze(ptemp(nTx,nTy,:));

QQ_u1 = utempT(t0:tident)'; %#ok<*USENS>
QQ_v1 = vtempT(t0:tident)';
QQ_p1 = ptempT(t0:tident);

valid.QQ_u1 = utempT(tident+1:end)';
valid.QQ_v1 = vtempT(tident+1:end)';
valid.QQ_p1 = ptempT(tident+1:end)';

if ~useVal
    idxPhiVal = phi1(tident+1:end) <= 40; 
end
tempQ_u1 = valid.QQ_u1(idxPhiVal);
tempQ_v1 = valid.QQ_v1(idxPhiVal);
tempQ_p1 = valid.QQ_p1(idxPhiVal);
valid.QQ_u1 = tempQ_u1(t0:end);
valid.QQ_v1 = tempQ_v1(t0:end);
valid.QQ_w1 = tempQ_v1(t0:end);
...
Psum = PT1(t0:tident)+PT2(t0:tident);

% figure; plot(abs(phi1(t0:tident)),Psum,'*')
% 
% figure; plot(cos(phi1(t0:tident)/180 *pi),Psum,'*')
% figure; plot(abs(phi1(t0:tident)),Psum,'*')
% 
% 
% figure; plot(cos(phi1(tval:end)/180 *pi),PT1(tval:end)+ PT2(tval:end),'*')
% figure; plot(abs(phi1(tval:end)),PT1(tval:end)+ PT2(tval:end),'*')
% figure; plot(abs(phi1(tval:end)/180 *pi),PT1(tval:end)+ PT2(tval:end),'*')
...
for idx = 1: length(noStates)
    ...
    if poly == 1 && yawmode == 0
        dirdmdName = 'eDMDresults_UasOutput_poly_PT';
    elseif poly == 0 && yawmode == 0
        dirdmdName = 'eDMDresults_UasOutput_PT';
    elseif poly == 1
        dirdmdName = 'eDMDresults_UasOutput_MIMO_poly_PT';
    else
        dirdmdName = 'eDMDresults_UasOutput_MIMO_PT';
    end
    ...
    folderNameVin = strrep(sprintf('Vin_Vinf%2.1f%s%s_states%02d',Vinf,Vstr,NTMstr,n),'.','dot');%'Vinf%d_diffComb_states%d'
    ...
    dirFigVin = fullfile(dirdmd,folderNameVin); % this depends on vinf
    if ~exist(dirFigVin,'dir')
        mkdir(dirFigVin);
    end
    ...
    statesP = [Outputs(1:2,:);states];
    statesvalidP = [Outputs_val(1:2,:)/10^6;statesvalid];
    stateNameP = ['P1;P2;',stateName];
    
    %     aTable = array2table(statesP');
    %     tmp = strsplit(stateNameP,';');
    %     aTable.Properties.VariableNames = tmp';
    %
    ...
    %         eDMD_RT_Vin_P(statesP,stateNameP,statesvalidP,poly,n+2,yawmode);
    
    [outStatesAndInputsP] = [sum(Outputs(1:2,:)); states];
    [outStatesAndInputsPvalid] = [sum(Outputs_val(1:2,:)); statesvalid];
    ...
    %eDMD_RT_Vin_P(statesP,stateNameP,statesvalidP,poly,n+2,yawmode);
    Vin = 0; 
    strVal = 'Id.'; displayOff = 0; Vinf = '';strOut = 'P';
    plotEDMDinputsEffWind(ysim,Inputs0,Deterministic0, FITje0,dirFig,Vin,n,strVal,displayOff,Vinf,strOut);
    strVal = 'Val.';
    plotEDMDinputsEffWind(ysim_val,Inputs_val0,Deterministic_val0,FITje_val0,dirFig,Vin,n,strVal,displayOff,Vinf,strOut);
    
    sys_red = sys_red0; FITje = FITje0; FITje_val = FITje_val0; xo = xo_0; Koop = Koop0;
    save(fullfile(dirFig,fileName),'sys_red','FITje','FITje_val','dirName','xo','Koop');
    
    
    % Sys id with ct1,ct2,V1 as inputs
    statesVinfP = [outStatesAndInputsP;statesUV(1,:)]; %[statesP;statesUV(1,:)];
    statesvalidVinfP = [outStatesAndInputsPvalid; statesUV_val(1,:)]; %[statesvalidP; statesUV_val(1,:)];
    stateNameVinfP =  [outStateName,';V1']; %[stateNameP,';V1'];
    
    [sys_red,FITje,Xd,Xd_p,xsim,FITje_val,xo,Koop,...
        ysim,InputsVinf,DeterministicVinf,ysim_val,Inputs_valOut,DeterministicVinf_val] = ...
        eDMD_RT_Vin_P(statesVinfP,stateNameVinfP,statesvalidVinfP,poly,n,yawmode);
    strVal = 'Id.'; Vin = 1;
    plotEDMDinputsEffWind(ysim,InputsVinf,DeterministicVinf, FITje,dirFigVin,Vin,n,strVal,displayOff,Vinf,strOut);
    strVal = 'Val.';
    plotEDMDinputsEffWind(ysim_val,Inputs_valOut,DeterministicVinf_val,FITje_val,dirFigVin,Vin,n,strVal,displayOff,Vinf,strOut);
    
%     statesVinfAll = [statesVinfP, statesUV(1,:)]; %[statesP;statesUV(1,:)];
%     statesvalidVinfP = [outStatesAndInputsPvalid; statesUV_val(1,:)]; %[statesvalidP; statesUV_val(1,:)];
%     stateNameVinfP =  [outStateName,';V1']; %[stateNameP,';V1'];
   
    
    save(fullfile(dirFigVin,fileName),'sys_red','FITje','FITje_val','dirName','xo','Koop');
...
plotEDMDinputsEffWind(ysim,Inputs,Deterministic, FITje,dirFigVin,Vin,n,strVal);
plotEDMDinputsEffWind(ysim_val,Inputs_val,Deterministic_val,FITje_val,dirFigVin,Vin,n);
