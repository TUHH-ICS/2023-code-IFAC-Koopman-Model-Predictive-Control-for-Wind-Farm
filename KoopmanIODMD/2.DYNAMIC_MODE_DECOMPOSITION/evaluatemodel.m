function [FITje,fig1,x] = evaluatemodel(sys_red,si,Inputs,Outputs,FITje,purpose,x,states,U,Deterministic,method,plotOn)
% dN=2;
...
if a
    ...
    elseif method == 2
       % [xo]=dinit(sys_red{si}.A,sys_red{si}.B,sys_red{si}.C,sys_red{si}.D,[Inputs]',[Outputs]');
         xo = U(1:size(states,1),1:si)'*states(:,1);% intial state % U(1:size(states,1),1:si)'*states(:,1)
    elseif method==3
    ...
    [ysim, ~, xout]=lsim(sys_red{si}, Inputs',[],xo);  
    x{si}=xout;
    ...
    FITje(:,si) = vaf(Outputs',ysim);% ToDo If all relevent frequencies have been taken into account
    if plotOn == 0
        return;
    end
    ...
    %% GRAPHICAL VISUALISAITON OF MODEL PREDICTION AND TRUE SIMULATION (VALIDATION) RESULTS
     ...
     [ysim_val, ~, xout]= lsim(sys_red{si}, Inputs',[],xo_val);  
