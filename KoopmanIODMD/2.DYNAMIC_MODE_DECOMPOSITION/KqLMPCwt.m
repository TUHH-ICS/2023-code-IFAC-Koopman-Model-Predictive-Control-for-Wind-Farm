function [Koop,XX,Uf,xkoop,add,timing]  = KqLMPCwt(Koop,x,x_,MPC,Uf,XX,lcp2,options,PolyLiftingFunction,noStates,structPrev)
...
for i = 0:options.Library_Update_Prediction_Horizon - 1
    ...
    Predicted_X_from_data_ext(:,i+2) = Khat *...
        K_psi([Predicted_X_from_data_ext(:,i+1); x_Matrix(nx+1:n,i+1)],... x_((i+1)*nx+1:(i+1)*nx+ni)],...
        PolyLiftingFunction,noStates,structPrev);    % Khat * Psi([xk_-1;uk_-1])
end
...
if error_norm > v
    ...
    nLength = length(x_)/options.Library_Update_Prediction_Horizon;
    matX_ = reshape(x_,options.Library_Update_Prediction_Horizon,nLength);
    matX = reshape(x,options.Library_Update_Prediction_Horizon,nLength);
    
%     psi_xk_1 = nan(noStates+ni,options.Library_Update_Prediction_Horizon);
%     for idx = 1: options.Library_Update_Prediction_Horizon
%         [psi_xk_1(:,idx),~,structPrev] = K_psi( matX_(:,idx), PolyLiftingFunction,noStates,structPrev);
%     end
% 
%     psi_xk = nan(noStates+ni,options.Library_Update_Prediction_Horizon);
%     for idx = 1: options.Library_Update_Prediction_Horizon
%         [psi_xk(:,idx),~,structPrev] = K_psi(matX(:,idx), PolyLiftingFunction,noStates,structPrev);
%     end

    P = P + 1;  % Number of library updates
    psi_xk_1 = K_psi(x_(end-n:end-1),PolyLiftingFunction,noStates,structPrev);% predict xk-1
    psi_xk = K_psi(x(end-n:end-1),PolyLiftingFunction,noStates,structPrev);% predict xk
...
approxC = [eye(nx),zeros(nx,noStates-nx)];
approxD = zeros(nx,ni);
sysC = d2c(ss(approxA,approxB,approxC,approxD,dt),'tustin');
add(3) = max(real(eig(sysC)));
if add(3) < 0.01 && add(2) == 1
    Koop.A = A;
    Koop.G = G;
    Koop.K = K;
    Koop.P = P;
elseif add(3) >= 0 && add(2) == 1
    add(2) = -1;    
else
    Koop.P = 1;
% elseif add(2) == 1 
...
Khat = Koop.K(:,1:nx)';  % Update the truncated Koopman operator

approxA = Koop.K(1:noStates,1:noStates); % system matrix
approxB = Koop.K(1:noStates,noStates+1:end);
approxC = [eye(nx),zeros(nx,noStates-nx)];
approxD = zeros(nx,ni);
sysC = d2c(ss(approxA,approxB,approxC,approxD,dt),'tustin');
% add(4) = max(real(eig(sysC)));
...
ph = K_psi(x(end-n+1:end),...
    PolyLiftingFunction,noStates,structPrev); % Calculate observables        
XXk = zeros(nx*NN1,1);       % Storage Matrix for all predicted xk+i
XXk(1:nx,1) = Khat*ph;         % Calculate one step ahead prediction 
xkoop = XXk(end-nx+1:end);

% % further predictions
% for i = 1:options.N_Prediction_Horizon - 1
%     ph = K_psi([XXk((i-1)*nx+1:i*nx); Uf((i-1)*ni+1:i*ni)],...
%         PolyLiftingFunction,noStates,structPrev);
%     
