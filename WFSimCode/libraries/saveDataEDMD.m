function [time, CT_prime, Phi, Power, Force, V, JR] = saveDataEDMD(Wp,sol_array,controller, ControlSetStr,DataOut,time0)
...
% Ct: 0.2-2: phi = 0:25
%mpc_R = mpc.R(1)*diag(repmat([ones(1,2),0.1], 1, length(grad_dU)/3));
%JR = mean(mean(abs(diff(CT_prime(:,time0:end)'))))/0.18 + mean(mean(abs(diff(Phi(:,time0:end)'))))/25;
JR = mean(mean(abs(diff(CT_prime(:,time0:end)'))))*2 + mean(mean(abs(diff(Phi(:,time0:end)'))))*0.1 +...
    mean(mean(abs(Phi(:,time0:end)'))) *10^(-3);
